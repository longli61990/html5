...
function href_join2(){
    window.location.href="join2.html?uid="+uid+"&sponsor="+sponsor+"&crid="+crid;
}
...
var Util = {
    countdown:{
        ...
        callback2:null,//每一秒的回调
        isstart:0,
        init:function(time,jdom,callback,callback2){
            this.jdom=jdom;
            this.time=time;
        ...
        start:function(){
                ...
                this.fun = setTimeout(function(){
                    Util.countdown.time--;
                    if(Util.countdown.time>=0){
                        if(Util.countdown.callback2)Util.countdown.callback2(Util.countdown.time);
                        Util.countdown.start()
                    }else{
    ...
    countdown2:{
        ...
        start:function(){
            ...
            this.fun = setTimeout(function(){
                _this.time--;
                if(_this.time>=0){
                    if(_this.callback2)_this.callback2(_this.time);
                    _this.start()
                }else{
                    _this.isstart=0;
    ...
    randoms:function(num1,num2,count){
        var data=[];
        if(num2-num1>=count){
            var chushu = Math.round((num2-num1+1)/count);
            for(var i=0;i<count;i++){
                var numStart=num1+chushu*i;
                if(i>0)numStart++;
                var numEnd=num1+chushu*(i+1);
                if(i==count-1)numEnd=num2;
                //alert(numStart+"  "+numEnd);
                data[i]=(Math.round(Math.random()*(numEnd-numStart)+numStart));
            }
        }
        return data;
    },
...
var app={
    ...
    get_player_status:function(callback){
        if(!variable.load_get_player_status){
            ...
            $.ajax({
                ...
                data:{"launch_id":launch_id,"user_id": uid,"crid":crid},
                timeout : http_timeout,
                type : 'get',
                dataType:'json',
                complete:function(XMLHttpRequest,status){
    ...
    user:function(callback){
        if(!variable.load_user){
            variable.load_user=true;
            $.ajax({
                url:serverUrl+"/user?callback=?",
                data:{"uid":uid},
                timeout : http_timeout,
                type : 'get',
                dataType:'json',
                complete:function(XMLHttpRequest,status){
                    variable.load_user=false;
                    if(callback)callback(status, XMLHttpRequest.responseJSON);
                }
            });
        }
