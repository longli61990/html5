...
var game={
    ...
    train_gamestate:function(callback){
        app.get_start_status(function(status,data){
            if (status=="success"&&data.ret == 0) {
                ...
                if(game.gamestate==2||game.gamestate==23){//is_start 0入场1倒计时2开始3结束
                    ...
                    if(!game.game_dom_pop.djs_pop.state && Util.countdown2.isstart==0){
                        game.game_dom_pop.djs_pop.dom.show();
                        Util.countdown2.init(3, game.game_dom_pop.djs_pop.children.go_djs.dom, function () {
                            game.game_dom_pop.djs_pop.children.go_djs.dom.html("GO");game.game_dom_pop.djs_pop.dom.addClass("ani_djspop");
                            game.game_dom_pop.djs_pop.state=1;
                            setTimeout(playGame,1000);
                        },function(time){game.game_dom_pop.djs_pop.children.go_djs.dom.hide();setTimeout(function(){game.game_dom_pop.djs_pop.children.go_djs.dom.show();},100);}).start();
                    }else if(game.game_dom_pop.djs_pop.state){
                        playGame();
                    }
                    ...
                    if(game.gamestate==0||game.gamestate==1) {
                        ...
                        for(var i=0;i<data.data.length;i++){
                            ...
                            if (i == 0) {//秀主
                                ...
                                html += '<li uid="'+user.player_id+'" class="read_' + user.player_join_status + '">';
                        ...
                        game.game_dom.div_before.children.before_seat.dom.html(html);
    ...
    kai:function(system_call){
        ...
        //console.log(win_user_id+"  "+lose_user_id+"  result_count="+result_count+"  kai_point_count="+kai_point_count);
